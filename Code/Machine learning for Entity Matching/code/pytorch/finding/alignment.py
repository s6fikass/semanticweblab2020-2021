...
from pytorch.utils import task_divide, merge_dic


def greedy_alignment(embed1, embed2, top_k, num_threads, metric, normalize, csls_k, accurate):
    """
    ...
    size = sim_mat.shape[0]
    if num_threads > 1:
        hits = [0] * len(top_k)
        mr, mrr = 0, 0
        alignment_rest = set()
        rests = []
        search_tasks = task_divide(np.arange(0, size), num_threads)
        pool = multiprocessing.Pool(processes=len(search_tasks))
        for task in search_tasks:
            mat = sim_mat[task, :]
            rests.append(pool.apply_async(calculate_rank, (task, mat, top_k, accurate, size)))
        pool.close()
        pool.join()
        for rest in rests:
        ...
        mr, mrr, hits, alignment_rest = calculate_rank(list(range(size)), sim_mat, top_k, accurate, size)
    assert len(alignment_rest) == size
    hits = np.array(hits) / size * 100
...
def stable_alignment(embed1, embed2, metric, normalize, csls_k, num_threads, cut=100, sim_mat=None):
    t = time.time()
    if sim_mat is None:
        sim_mat = sim(embed1, embed2, metric=metric, normalize=normalize, csls_k=csls_k)

    kg1_candidates, kg2_candidates = {}, {}
    size = sim_mat.shape[0]

    x_tasks = task_divide(np.arange(0, size), num_threads)
    ...
    sim_mat = sim_mat.T
    size = sim_mat.shape[0]
    y_tasks = task_divide(np.arange(0, size), num_threads)
...
def calculate_rank(idx, sim_mat, top_k, accurate, size):
    assert 1 in top_k
    mr = 0
    mrr = 0
    hits = [0] * len(top_k)
...
def retrieve_topk_alignment(kg1_source_ents, embeds1, kg2_candidates, embeds2, k=1, metric='inner',
                            normalize=False, csls_k=0, output_path=None):
